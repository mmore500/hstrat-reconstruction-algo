...
real_sources = [
    "${HOME}/2026-03-09/a=fossils+flavor=pure+ext=.pqt",
    "${HOME}/2026-03-09/a=fossils+flavor=plus+ext=.pqt",
]
...
stdbuf -e0 -i0 -o0 echo "/local/\$(basename "\${genomes_inpath}")" \
    ...
    | stdbuf -e0 -i0 -o0 python3.8 -m pylib.script.tee_eval_context_durations \
        ...
        --with-column 'pl.lit(96).alias("dstream_S")' \
...
echo "PHYLOFRAME_CONTAINER ${PHYLOFRAME_CONTAINER}"
singularity exec ${PHYLOFRAME_CONTAINER} \
    python3 -m phyloframe.legacy._alifestd_as_newick_polars \
...
if [ "${ACTION}" = "submit" ] || [ "${ACTION}" = "submit-downsample" ]; then
    ...
    for ntips in 100 500 1_000 1_500 2_000 2_500 3_000 3_500 4_000 4_500; do
        ntips_label="${ntips//_/}"  # strip underscores for label
        dsamp_labels+=("clade${ntips_label}")
        dsamp_outnames+=("a=phylo+dsamp=clade${ntips_label}+ext=")
        dsamp_modules+=("_alifestd_downsample_tips_clade_asexual")
        dsamp_args+=("-n ${ntips} --seed 1")
    done
