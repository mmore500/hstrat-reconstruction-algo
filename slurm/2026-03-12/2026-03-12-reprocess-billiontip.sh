...
################################################################################
HSTRAT_CONTAINER="docker://ghcr.io/mmore500/hstrat:v1.25.0"
PHYLOFRAME_CONTAINER="docker://ghcr.io/mmore500/phyloframe:v0.2.1"
...
real_sources = [
    "${HOME}/2026-03-10-reprocess/a=fossils+flavor=pure+ext=.pqt",
    "${HOME}/2026-03-10-reprocess/a=fossils+flavor=plus+ext=.pqt",
]
...
stdbuf -e0 -i0 -o0 echo "/local/\$(basename "\${genomes_inpath}")" \
    | stdbuf -o0 singularity exec ${HSTRAT_CONTAINER} \
        ...
        --filter "pl.col('bookend_value').is_not_null()" \
        2>&1 \
...
if [ "${ACTION}" = "submit" ] || [ "${ACTION}" = "submit-downsample" ]; then
    ...
    for ntips_k in 0.1k 0.3k 1.0k 3k 10k 30k; do
