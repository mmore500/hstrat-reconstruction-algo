...
echo "set up container venv --------------------------------------- \${SECONDS}"
git clone https://github.com/mmore500/hstrat.git
cd hstrat
...
# initial DataFrame
df = pl.read_parquet("osf-hm5wa.pqt")

# Perform the operations
df = (
    df.with_columns([
        pl.col("bitfield").alias("data_hex"),  # Rename bitfield to data_hex
        pl.lit(32).alias("dstream_T_bitoffset"),  # Add a new column with value 32
        pl.lit(32).alias("dstream_T_bitwidth"),   # Add another new column with value 32
        pl.lit(64).alias("dstream_storage_bitoffset"),  # Add column with value 64
        pl.lit(64).alias("dstream_storage_bitwidth"),   # Add another column with value 64
        pl.lit(64).alias("dstream_S"),                 # Add column with value 64
        pl.lit("dstream.tilted_algo").alias("dstream_algo").cast(pl.Categorical)  # Add categorical column
    ])
    .drop(["bitfield", "replicate", "nCycle", "msec", "tsc"])  # Drop specified columns
)
...
def measure_execution_time(head_count: int):
    # use only the head of the dataframe as specified by head_count
    sub_df = df.head(head_count)

    t1 = time.perf_counter(), time.process_time()
    surface_unpack_reconstruct(sub_df)
...
df_records.to_csv(outpath, index=False)



EOF_

echo "clean up ---------------------------------------------------- \${SECONDS}"
rm -rf hstrat
rm -rf senv
