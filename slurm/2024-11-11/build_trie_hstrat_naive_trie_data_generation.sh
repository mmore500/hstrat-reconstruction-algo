...
HSTRAT_REMOTE_URL="https://github.com/mmore500/hstrat-reconstruction-algo.git"
...
csv_files = [
    "https://raw.githubusercontent.com/mmore500/hstrat/refs/heads/master/tests/test_hstrat/assets/nk_ecoeaselection.csv",
    "https://raw.githubusercontent.com/mmore500/hstrat/refs/heads/master/tests/test_hstrat/assets/nk_lexicaseselection.csv",
    "https://raw.githubusercontent.com/mmore500/hstrat/refs/heads/master/tests/test_hstrat/assets/nk_tournamentselection.csv"
]

# Function to load CSV files from URLs into DataFrames, making it easy to test different sample trees
def load_csv(file_url):
    print(f"Loading file from URL: {file_url}")
    df = pd.read_csv(file_url)
    print("Columns in CSV:", df.columns)

    # Add a new column to indicate the selection type
    if "nk_ecoeaselection" in file_url:
        df["selection_type"] = "nk_ecoeaselection"
    elif "nk_lexicaseselection" in file_url:
        df["selection_type"] = "nk_lexicaseselection"
    elif "nk_tournamentselection" in file_url:
        df["selection_type"] = "nk_tournamentselection"
    return df

# Function to benchmark the `build_tree_trie` method by timing its execution
def benchmark_build_tree_trie(population, repetitions=10, selection_type=""):
    records = []  # List to store benchmarking results

    # Perform the benchmarking the specified number of times
    for _ in range(repetitions):
        # Start timing
        start_real_time = time.perf_counter()
        start_cpu_time = time.process_time()

        # Call the `build_tree_trie` function
        tree = hstrat.build_tree_trie(
            population,
            force_common_ancestry=True  # Ensure that the population has a common ancestry
        )
...
# Main loop to benchmark the `build_tree_trie` function
results = []

# Create a fixed resolution policy to avoid errors
default_policy = hstrat.fixed_resolution_algo.Policy(3)

# Iterate through each CSV file to load the population data and benchmark the function
for file_url in csv_files:
    # Load the population data from the CSV
    population_data = load_csv(file_url)

    # Determine the selection type for labeling purposes
    if "nk_ecoeaselection" in file_url:
        selection_type = "nk_ecoeaselection"
    elif "nk_lexicaseselection" in file_url:
        selection_type = "nk_lexicaseselection"
    elif "nk_tournamentselection" in file_url:
        selection_type = "nk_tournamentselection"

    # Create a list to represent the population based on the loaded data
    population = []
    for index, row in population_data.iterrows():
        column = hstrat.HereditaryStratigraphicColumn(stratum_retention_policy=default_policy, stratum_differentia_bit_width=16)
        # Add strata to the column based on the depth information from the CSV
        column.DepositStrata(int(row['depth']))
        population.append(column)

    # Benchmark `build_tree_trie` without any additional postprocessing
    results += benchmark_build_tree_trie(population, selection_type=selection_type)

# Convert the benchmark results to a DataFrame and save them as a CSV file
benchmark_df = pd.DataFrame(results)
benchmark_df["SLURM_ARRAY_TASK_ID"] = "\${SLURM_ARRAY_TASK_ID:-notid}"
