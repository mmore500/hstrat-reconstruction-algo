...
echo "\${MYLOCAL}/\$(basename "\${genomes_inpath}")" | \
        python3.12 -O -m hstrat.dataframe.surface_unpack_reconstruct \
        "/tmp/\$(uuidgen).pqt" \
        --head 100
...
echo "\${MYLOCAL}/\$(basename "\${genomes_inpath}")" | \
        python3.12 -O -m hstrat.dataframe.surface_unpack_reconstruct \
        "\${MYLOCAL}/\$(basename "\${reconst_outpath}")" \
...
cp "\${reconst_outpath}" "\${JOBDIR}/a=phylo+ext=.pqt"
